...
public record DisseminationMarker(Disseminations type, List<String> countries) {
    ...
    public DisseminationMarker {
        ...
        if (!countries.isEmpty() && !List.of(Disseminations.RELEASE_TO, Disseminations.DISPLAY_ONLY).contains(type)) {
            ...
            buf.append(Disseminations.RELEASE_TO.toString()).append("' and '").append(Disseminations.DISPLAY_ONLY.toString());
            ...
            throw new IllegalArgumentException("The '" + Disseminations.DISPLAY_ONLY.toString() + "' mark requires at least 1 country.");
        }
        else if (type == Disseminations.RELEASE_TO) {
            if (!countries.contains(Utils.USA)) {
                throw new IllegalArgumentException("For the '" + Disseminations.RELEASE_TO.toString() + "' mark, the countries list must include " + Utils.USA + ".");
            }
            else if (countries.size() < 2) {
                throw new IllegalArgumentException("For the '" + Disseminations.RELEASE_TO.toString() + "' mark, the countries list must have at least 2 entries.");
            }
        }
